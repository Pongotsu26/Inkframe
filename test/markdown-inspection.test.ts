...
describe("inspectMarkdown", () => {
  ...
  it("見出し記号の後に空白がない場合だけ警告する", () => {
    ...
    expect(inspection.issues).toContainEqual(expect.objectContaining({ severity: "warning", line: 1 }));
  });

  it("空の見出しは空白不足として扱わない", () => {
    const inspection = inspectMarkdown("###");
    expect(inspection.issues.some(issue => issue.severity === "warning")).toBe(false);
  });
});
