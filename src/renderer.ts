...
function margins(value: string): { top: string; right: string; bottom: string; left: string } {
  const parts = value.trim().split(/\s+/);
  if (parts.length === 1) return { top: parts[0], right: parts[0], bottom: parts[0], left: parts[0] };
  if (parts.length === 2) return { top: parts[0], right: parts[1], bottom: parts[0], left: parts[1] };
  if (parts.length === 3) return { top: parts[0], right: parts[1], bottom: parts[2], left: parts[1] };
  if (parts.length === 4) return { top: parts[0], right: parts[1], bottom: parts[2], left: parts[3] };
  throw new Error("余白は CSS と同じ形式で 1〜4 個の値を指定してください（例: 20mm 18mm）");
}
...
function footerTemplate(value?: string, pageNumber?: boolean): string {
  const text = value ?? (pageNumber ? "<span class=\"pageNumber\"></span> / <span class=\"totalPages\"></span>" : "");
...
async function ghostscript(args: string[]): Promise<void> {
  try {
    ...
    throw new Error(`Ghostscript を実行できません。PDF の結合・圧縮には Ghostscript をインストールしてください。${error instanceof Error ? `\n${error.message}` : ""}`);
  }
}

export async function mergePdfs(inputs: string[], outputPath: string): Promise<void> {
  if (inputs.length === 0) throw new Error("結合する PDF を指定してください。");
  await mkdir(dirname(outputPath), { recursive: true });
  await ghostscript(["-q", "-dBATCH", "-dNOPAUSE", "-sDEVICE=pdfwrite", `-sOutputFile=${outputPath}`, ...inputs]);
}

export async function compressPdf(inputPath: string, outputPath: string, imageQuality?: number): Promise<void> {
  await mkdir(dirname(outputPath), { recursive: true });
  ...
  await ghostscript([
    ...
    "-dPDFSETTINGS=/printer", "-dDownsampleColorImages=true", "-dDownsampleGrayImages=true", "-dDownsampleMonoImages=true",
    `-dColorImageResolution=${resolution}`, `-dGrayImageResolution=${resolution}`, `-sOutputFile=${outputPath}`, inputPath
  ]);
}

/** Convert one Markdown document entirely on the local machine. */
export async function convertMarkdown(input: string, options: ConvertOptions = {}): Promise<string> {
  const inputPath = resolve(input);
  const outputPath = resolve(options.output ?? inputPath.replace(/\.(?:md|markdown)$/i, ".pdf"));
  ...
  try {
    ...
    throw new Error(`Chromium を起動できません。\`npx playwright install chromium\` を実行してください。\n${error instanceof Error ? error.message : String(error)}`);
  }
  try {
    const page = await browser.newPage();
    if (!settings.allowExternalResources) {
      await page.route("**/*", (route) => /^(?:file:|data:|blob:|about:)/.test(route.request().url()) ? route.continue() : route.abort());
    }
    await page.setContent(document.html, { waitUntil: "load" });
    if (settings.mermaid !== false && document.html.includes('class="mermaid"')) {
      await page.addScriptTag({ path: document.mermaidScriptPath });
      await page.evaluate(async () => {
        const mermaid = (window as unknown as { mermaid: { initialize: (config: object) => void; run: () => Promise<void> } }).mermaid;
  ...
  if (options.compress || options.imageOptimize) {
    ...
    await compressPdf(outputPath, temporary, options.imageOptimize ? options.imageQuality : undefined);
...
export async function buildMarkdownFiles(inputs: string[], output: string, options: ConvertOptions = {}): Promise<string> {
  if (inputs.length === 0) throw new Error("結合する Markdown ファイルを指定してください。");
  const temporary = await mkdtemp(join(tmpdir(), "mdpdf-build-"));
  try {
    const generated: string[] = [];
    for (const [index, input] of inputs.entries()) {
      const pdf = join(temporary, `${String(index + 1).padStart(3, "0")}.pdf`);
      generated.push(await convertMarkdown(input, { ...options, output: pdf, compress: false, imageOptimize: false }));
    }
    const outputPath = resolve(output);
    await mergePdfs(generated, outputPath);
    if (options.compress || options.imageOptimize) await compressPdf(outputPath, `${outputPath}.optimized.pdf`, options.imageOptimize ? options.imageQuality : undefined).then(async () => {
...
export async function markdownFilesIn(directory: string): Promise<string[]> {
  ...
  async function walk(current: string): Promise<void> {
    for (const entry of await readdir(current, { withFileTypes: true })) {
      ...
      else if (entry.isFile() && /\.(?:md|markdown)$/i.test(extname(entry.name))) result.push(path);
    }
  }
