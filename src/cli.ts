...
import { buildMarkdownFiles, convertMarkdown, markdownFilesIn, mergePdfs, paper, type ConvertOptions } from "./renderer.js";
...
function optionsConfig(options: CliOptions): ConvertOptions {
  return {
    ...
    font: { body: options.bodyFont ?? options.font?.body, heading: options.headingFont ?? options.font?.heading, code: options.codeFont ?? options.font?.code }
  };
}
...
async function outputFor(input: string, outDirectory: string): Promise<string> {
  ...
  return join(resolve(outDirectory), `${basename(input).replace(/\.(?:md|markdown)$/i, "")}.pdf`);
}
...
program.name("inkframe").enablePositionalOptions().description("Markdownを美しいPDFに仕上げるローカル組版CLI").version("0.2.0")
  ...
  .option("--allow-external-resources", "外部 URL の画像・スタイル等の読み込みを許可")
  .action(async (input: string | undefined, options: CliOptions) => {
    if (!input) throw new Error("入力する Markdown ファイルを指定してください。");
    report(await convertMarkdown(input, optionsConfig(options)));
  });

program.command("fonts").description("利用可能なローカルフォントを一覧表示").action(async () => {
...
program.command("batch <directory>").description("フォルダ内の Markdown を再帰的に PDF 化").requiredOption("--out <directory>", "出力先ディレクトリ")
  ...
  .action(async (directory: string, options: CliOptions & { out: string }) => {
    ...
    for (const file of files) report(await convertMarkdown(file, { ...optionsConfig(options), output: await outputFor(file, options.out) }));
  });
...
program.command("watch <input>").description("保存時に PDF を再生成").option("-o, --output <path>", "出力 PDF のパス").option("--theme <name>").option("--paper <size>").option("--toc").option("--page-number").option("--compress")
  .action(async (input: string, options: CliOptions) => {
    ...
    const generate = async (): Promise<void> => {
      try { report(await convertMarkdown(input, optionsConfig(options))); } catch (error) { console.error(error instanceof Error ? `エラー: ${error.message}` : error); }
    };
    await generate();
    console.log(`監視中: ${resolve(input)}（Ctrl+C で終了）`);
    watchFile(resolve(input), () => { if (timer) clearTimeout(timer); timer = setTimeout(() => void generate(), 250); });
  });

program.parseAsync().catch((error: unknown) => { console.error(error instanceof Error ? `エラー: ${error.message}` : error); process.exitCode = 1; });
