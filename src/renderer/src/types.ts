...
export interface MdpdfApi {
  ...
  watch(path?: string): Promise<boolean>; inspect(content: string): Promise<Inspection>; renderPreview(content: string, path: string | undefined, options: ConvertOptions): Promise<string>;
